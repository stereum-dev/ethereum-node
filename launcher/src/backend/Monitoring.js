...
export class Monitoring {
  ...
  async getServiceInfos() {
    ...
    const file = path.join(
      os.tmpdir(),
      "server_infos_cache_" + process.getCreationTime() + ".txt"
    );
    const dnow = new Date();
    var cont = {};
  ...
  async queryRpcApi(method, params = [], ec_only = false) {
    ...
    const execution = serviceInfos
      .filter((s) => s.config.ports.filter((p) => p.servicePort == 8545).pop())
      .pop();
    ...
    if (data.api_reponse.hasOwnProperty("error")) {
      data.api_reponse =
        data.api_reponse.error.message +
        " (" +
        data.api_reponse.error.code +
        ")";
      return {
        code: 8,
  ...
  async queryBeaconApi(
    ...
    const consensus = serviceInfos
      .filter((s) => Object.keys(services).includes(s.service))
      .pop();
    ...
    headers =
      typeof headers === "object" && !Array.isArray(headers) && headers !== null
        ? headers
        : {};
    headers = {
      ...Object.fromEntries(
    ...
    // Check for response errors
    if (
      data.api_reponse.hasOwnProperty("code") &&
      data.api_reponse.hasOwnProperty("message")
    ) {
      data.api_reponse =
        data.api_reponse.message + " (" + data.api_reponse.code + ")";
      return {
        code: 7,
        info: "error: api responded an error -> " + data.api_reponse,
        data: data,
  ...
  async getSyncStatus() {
    ...
    const services = {
      consensus: {
        ...
        ], // OK - requires query for job="lighthouse_beacon"!
      ...
      execution: {
        ...
        ], // OK - optional query for job="besu"
    ...
    try {
      ...
      clientTypes.forEach(function (clientType, index) {
        ...
        let xx = prometheus_result.data.result.filter((s) =>
          ...
          clt.service == "TekuBeaconService"
            ? s.metric.job == "teku"
            : true && clt.service == "LighthouseBeaconService"
            ? s.metric.job == "lighthouse_beacon"
            : true && clt.service == "PrysmBeaconService"
            ? s.metric.job == "prysm_beacon"
            : true && clt.service == "NimbusBeaconService"
            ? s.metric.job == "nimbus"
            : true && clt.service == "GethService"
            ? s.metric.job == "geth"
            : true && clt.service == "BesuService"
            ? s.metric.job == "besu"
            : true && clt.service == "NethermindService"
            ? s.metric.job == "nethermind"
            : true
        );
        if (xx.length) {
        ...
        data.push({
          ...
          type: clientType,
          state: clt.state,
          uptime: clt.createdAt
            ? utsNow - Math.floor(new Date(clt.createdAt).getTime() / 1000)
            : 0,
        });
      });
  ...
  async openRpcTunnel(args) {
    ...
    if (this.rpcTunnel > 0 && !force_fresh) {
      rpcstatus.data.url =
        "http://" + rpcstatus.data.rpc.destinationIp + ":" + this.rpcTunnel;
      return {
        code: 0,
    ...
    try {
      var localPort =
        typeof force_local_port == "number" && force_local_port > 0
          ? force_local_port
          : 8545;
  ...
  async getRpcStatus() {
    ...
    return {
      ...
      data: {
        ...
        url:
          this.rpcTunnel > 0
            ? "http://" + result.data.rpc.destinationIp + ":" + this.rpcTunnel
            : "",
  ...
  async openBeaconTunnel(args) {
    ...
    if (this.beaconTunnel > 0 && !force_fresh) {
      beaconstatus.data.url =
        "http://" +
        beaconstatus.data.beacon.destinationIp +
        ":" +
        this.beaconTunnel;
      return {
        code: 0,
    ...
    return {
      ...
      data: {
        ...
        url:
          this.beaconTunnel > 0
            ? "http://" +
              result.data.beacon.destinationIp +
              ":" +
              this.beaconTunnel
            : "",
        clt: result.data.cc.service
          .replace(/Beacon|Service/gi, "")
          .toUpperCase(),
      },
    };
