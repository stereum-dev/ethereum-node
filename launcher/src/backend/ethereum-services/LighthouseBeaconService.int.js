...
test("lighthouse validator import", async () => {
  ...
  let lhBC = LighthouseBeaconService.buildByUserInput(
    "goerli",
    ports,
    nodeConnection.settings.stereum.settings.controls_install_path + "/lighthouse",
    [geth],
    "16",
    []
  );
  ...
  let lhVC = LighthouseValidatorService.buildByUserInput(
    "goerli",
    ports,
    nodeConnection.settings.stereum.settings.controls_install_path + "/lighthouse",
    [lhBC]
