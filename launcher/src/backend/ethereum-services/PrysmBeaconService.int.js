...
test('prysm validator import', async () => {
    ...
    const extraVars = {
        stereum_role: 'validator-import-api', consClient: 'prysm', auth_token_path: wallet_path + '/auth-token', validator_service: prysmVC.id, validator_keys:
    ...
    const VCstatus = await nodeConnection.sshService.exec(`docker logs --tail=70 stereum-${prysmVC.id}`)
