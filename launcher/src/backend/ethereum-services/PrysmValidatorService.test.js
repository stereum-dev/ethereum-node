...
test('buildConfiguration', () => {
    ...
    PrysmBeaconService.PrysmBeaconService.mockImplementation(() => {
      return {
        buildConsensusClientEndpoint: jest.fn(() => {return 'buildConsensusClientEndpoint'}),
        buildConsensusClientGateway: jest.fn(() => {return 'buildConsensusClientGateway'}),
    ...
    const prysm = PrysmValidatorService.buildByUserInput(networks.prater, ports, '/opt/stereum/prysm', [new PrysmBeaconService.PrysmBeaconService()]).buildConfiguration()
  ...
  test('getAvailablePorts', () => {
    ...
    const prysm = PrysmValidatorService.buildByUserInput(networks.prater, ports, '/opt/stereum/prysm', []).getAvailablePorts()
  ...
  test('autoupdate', () => {
    ...
    const prysm = PrysmValidatorService.buildByUserInput(networks.prater, ports, '/opt/stereum/prysm', []).buildConfiguration()
  ...
  test('buildByConfiguration', () => {
    ...
    const prysm = PrysmValidatorService.buildByConfiguration({
      ...
      '--accept-terms-of-use=true --beacon-rpc-provider="' + provider + 
      '" --beacon-rpc-gateway-provider="' + providerGateway + 
      '" --web=true --' + network + '=true --datadir=' + dataDir + 
      ' --wallet-dir=' + walletDir + ' --wallet-password-file=' + passwordDir + 
