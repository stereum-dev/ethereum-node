...
test('nimbus validator import', async () => {
    ...
    const serverSettings = {
        name: 'Nimbus--integration-test--ubuntu-2004',
        image: 'ubuntu-20.04',
        location: 'fsn1',
        server_type: 'cx11',
    ...
    //import validator
    const extraVars = { stereum_role: 'validator-import-api', consClient: 'nimbus', validator_password: 'MyTestPassword', beacon_service: nimbusClient.id, validator_keys: 
    ...
    //get logs
    const status = await nodeConnection.sshService.exec(`docker logs stereum-${nimbusClient.id}`)
    log.debug(status.stdout)
