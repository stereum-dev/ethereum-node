...
test("nimbus validator import", async () => {
  ...
  let nimbusClient = NimbusBeaconService.buildByUserInput(
    "goerli",
    ports,
    nodeConnection.settings.stereum.settings.controls_install_path + "/nimbus",
    [geth],
    []
  );
  ...
  await nodeConnection.sshService.exec(
    `echo ${token} > /opt/stereum/nimbus-${nimbusClient.id}/validator/validators/api-token.txt`
  );
  ...
  const extraVars = {
    ...
    validator_service: nimbusClient.id,
    validator_keys: [
      {
        name: "batch0",
