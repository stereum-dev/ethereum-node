...
test("lodestar validator import", async () => {
  ...
  let lBC = LodestarBeaconService.buildByUserInput(
    "goerli",
    ports,
    nodeConnection.settings.stereum.settings.controls_install_path + "/lodestar",
    [geth],
    []
  );
  ...
  let lVC = LodestarValidatorService.buildByUserInput(
    "goerli",
    ports,
    nodeConnection.settings.stereum.settings.controls_install_path + "/lodestar",
    [lBC]
  ...
  if (![lBC.id, lVC.id].join("").includes("Up")) {
