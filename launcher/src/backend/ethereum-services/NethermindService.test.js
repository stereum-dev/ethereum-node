...
test('buildByUserInput', () => {
    const ports = [
        new ServicePort(null, 4040, 4040, servicePortProtocol.tcp)
    ]

    const nethermindService = NethermindService.buildByUserInput(networks.goerli, ports, '/opt/stereum/nethermind').buildConfiguration()
...
test('buildExecutionClientMetricsEndpoint', () => {
    const nethermindMetricsEndpoint = NethermindService.buildByUserInput(networks.goerli, [], '/opt/stereum/nethermind').buildExecutionClientMetricsEndpoint()
    expect(nethermindMetricsEndpoint).toMatch(/stereum-.{36}:6060/)
})

test('buildPrometheusJob', () => {
    const nethermindPrometheusJob = NethermindService.buildByUserInput(networks.goerli, [], '/opt/stereum/nethermind').buildPrometheusJob()
    expect(nethermindPrometheusJob).toMatch(/\n  - job_name: stereum-.{36}\n    static_configs:\n      - targets: \[stereum-.{36}:6060]/)
})
