...
test("teku validator import", async () => {
  ...
  let tekuClient = TekuBeaconService.buildByUserInput(
    "goerli",
    ports,
    nodeConnection.settings.stereum.settings.controls_install_path + "/teku",
    [geth],
    []
  );
  ...
  await nodeConnection.sshService.exec(
    `cd ${dataDir} && keytool -genkeypair -keystore teku_api_keystore -storetype PKCS12 -storepass ${password} -keyalg RSA -keysize 2048 -validity 109500 -dname "CN=teku, OU=MyCompanyUnit, O=MyCompany, L=MyCity, ST=MyState, C=AU" -ext "SAN=DNS:stereum-${tekuClient.id}"`
  );
  ...
  const extraVars = {
    ...
    validator_service: tekuClient.id,
    validator_keys: [
      {
        name: "batch0",
