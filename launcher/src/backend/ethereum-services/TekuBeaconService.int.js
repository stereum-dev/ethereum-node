...
test('teku validator import', async () => {
    ...
    await nodeConnection.sshService.exec(`sudo echo ${password} > ${dataDir}/teku_api_password.txt`)
    await nodeConnection.sshService.exec(`sudo bash -c "cd ${dataDir} && keytool -genkeypair -keystore teku_api_keystore -storetype PKCS12 -storepass ${password} -keyalg RSA -keysize 2048 -validity 109500 -dname 'CN=localhost, OU=MyCompanyUnit, O=MyCompany, L=MyCity, ST=MyState, C=AU' -ext san=dns:localhost,ip:127.0.0.1"`)
    
    await testServer.Sleep(30000)
    //import validator
    const extraVars = {
        stereum_role: 'validator-import-api', consClient: 'teku', validator_password: 'MyTestPassword', beacon_service: tekuClient.id, validator_keys:
