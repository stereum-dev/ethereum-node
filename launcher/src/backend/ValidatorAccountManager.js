...
export class ValidatorAccountManager {
  ...
  createBatch(files, password, slashingDB) {
    ...
    if (slashingDB)
      var slashing_protection_content = readFileSync(slashingDB, {
        encoding: "utf8",
      });
    ...
    let batch = {
      ...
      passwords: passwords,
      content: content,
      ...(slashing_protection_content && {
        slashing_protection_content: slashing_protection_content,
      }), // READ ME & REMOVE ME short explanation !!! ---> for Martin, if slashing protection data is defined, then batch will use pass, content also slashing_p_db, if not defined, then only pass and content will be used ----> short explanation for Martin
    };
  ...
  async importKey(files, password, serviceID, slashingDB) {
    ...
    try {
      let run = await this.nodeConnection.runPlaybook("validator-import-api", {
        stereum_role: "validator-import-api",
        validator_service: client.id,
        validator_keys: this.batches,
      });
  ...
  async listValidators(serviceID) {
    try {
      let run = await this.nodeConnection.runPlaybook("validator-list-api", {
        stereum_role: "validator-list-api",
        validator_service: serviceID,
      });
      let logs = new RegExp(/^DATA: ({"msg":.*)/, "gm").exec(
        await this.nodeConnection.playbookStatus(run.playbookRunRef)
      );
  ...
  async deleteValidators(serviceID, keys, picked) {
    try {
      let run = await this.nodeConnection.runPlaybook("validator-delete-api", {
        stereum_role: "validator-delete-api",
        validator_service: serviceID,
        validator_public_keys: [{ pubkeys: keys }],
      });
      let logs = new RegExp(/^DATA: ({"msg":.*)/, "gm").exec(
        await this.nodeConnection.playbookStatus(run.playbookRunRef)
      );
