<template>
  ...
  >
    <div
      v-if="alertMessage !== ''"
      role="alert"
      class="absolute -top-[80px] left-14 w-2/3 max-h-36 rounded border-s-4 p-4 z-50 bg-red-100 border-red-400"
    >
    ...
    <div class="w-full h-full grid grid-cols-12 grid-rows-1">
      ...
      <input
        id="input1"
        v-model="groupName"
        class="col-start-2 col-end-11 w-full bg-[#171D22] border px-4 py-1 rounded-sm outline-none text-xs text-gray-400 border-gray-500 placeholder:text-gray-400"
        type="text"
        autofocus
        :class="inputClass"
      ...
      <div class="col-start-12 col-span-1 flex justify-center items-center p-1">
        <div
          class="w-6 h-6 rounded-md bg-[#171D22] p-1 flex justify-center items-center hover:scale-110 border border-[#171D22] active:scale-100 hover:shadow-md hover:shadow-[#101214] hover:border-[#3f4851] active:shadow-none transition-all duration-150"
          :class="isGroupNameValid ? 'cursor-pointer' : 'pointer-events-none opacity-50'"
