<template>
  <div
    class="w-full h-full border border-gray-600 rounded-[4px] grid grid-cols-7 items-center"
  >
    <div
    ...
    >
      <div class="w-full flex justify-between items-center">
        <img
          ...
          @mousedown.prevent
        />
        <span class="w-full text-[12px] text-green-600 font-semibold text-center">{{
          totalKeys
        }}</span>
      </div>
      <span class="w-1/3 text-[10px] text-amber-300 font-semibold text-center">{{
        $t("stakingPage.ttl")
      }}</span>
    </div>
    <div
      ...
      @mouseleave="footerStore.cursorLocation = ''"
    >
      <span class="text-[11px] text-gray-300 font-normal self-center text-center">{{
        totalBalance
      }}</span>
    </div>
  </div>
...
const totalBalance = computed(() => {
  ...
  const keysForValidator = stakingStore.keys.filter(
    (key) => key.validatorID === validatorID
  );
