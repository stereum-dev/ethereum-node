...
const updateProperties = () => {
  props.properties.executionClients = useDeepClone(
    list.value.filter((e) => e.category === "execution" && e.isConnected)
  );
  props.properties.consensusClients = useDeepClone(
    list.value.filter(
      (e) =>
        (e.category === "consensus" || e.service === "CharonService") && e.isConnected
    )
  );
...
const getConnectedClient = () => {
  list.value.forEach((service) => {
    if (service.config?.dependencies) {
      ...
      if (
        allDependencies &&
        allDependencies.map((s) => s.id).includes(service.config.serviceID)
      ) {
    ...
    case "consensus":
      if (props.client.service === "OpNodeBeaconService") {
        return manageStore.newConfiguration.filter(
          (e) =>
            (e.category === "execution" && e.name !== "L2Geth") ||
            (e.category === "consensus" && !e.service.includes("Op"))
        );
      }
