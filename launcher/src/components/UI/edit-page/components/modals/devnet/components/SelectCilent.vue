        ...
        <template v-for="service in sortedServices" :key="service.service">
          <div class="col-start-1 col-end-2 flex items-center">
            <img
              ...
              class="w-6 h-6 mr-2"
            />
            <span class="text-sm uppercase text-gray-300"
              >{{ service.category }} Client</span
            >
          </div>
          <div class="col-start-2 col-span-full grid grid-cols-4 items-center">
            <img
              :src="service.icon"
              alt="Service Icon"
              class="col-start-1 col-span-1 w-8 h-8"
            />
            <span class="col-start-2 col-span-full text-sm font-semibold text-gray-300">{{
              service.service
            }}</span>
          </div>
        </template>
...
onUnmounted(() => {
  setupStore.devnetConfigData.services = useDeepClone(sortedServices.value);
});
</script>
