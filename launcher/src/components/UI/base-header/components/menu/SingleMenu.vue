...
<template>
  ...
  >
    <img
      v-if="props.item.activeIcon && (headerStore.isUpdateAvailable || headerStore.isOsUpdateAvailable)"
      class="w-8"
      :src="props.item.activeIcon"
      alt="icon"
      @mousedown.prevent
    />
    <img v-else class="w-8" :src="props.item.icon" alt="icon" @mousedown.prevent />
  </div>
</template>
