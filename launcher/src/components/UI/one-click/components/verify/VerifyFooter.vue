<template>
  ...
  >
    ...
    >
      <span class="text-gray-200 text-xl font-semibold text-center uppercase">{{
        $t("customFooter.back")
      }}</span>
    </router-link>
    <div
...
const emit = defineEmits(["startInstalling"]);

const installStore = useClickInstall();

const goToBack = computed(() => {
  return installStore.selectedPreset.name === "op full node" ||
    installStore.selectedPreset.name === "op node archive"
    ? "/oneClick/config"
    : "/oneClick/sync";
});
