...
const updatePlugins = () => {
  if (
    ...
    const OpRethExists = clickStore.selectedPreset.includedPlugins.some(
      (item) => item.service === "OpRethService"
    );
    const L2GethExists = clickStore.selectedPreset.includedPlugins.some(
...
const pluginChangeHandler = (plugin, item, idx) => {
  ...
  if (
    ...
    if (item.category === "consensus" && getCorrespondingValidator(item.name)) {
      ...
      clickStore.selectedPreset.includedPlugins[valIndex] = getCorrespondingValidator(
        item.name
      );
    } else if (
      ...
      clickStore.selectedPreset.includedPlugins[conIndex] = getCorrespondingConsensus(
        item.name
      );
    }
  }
...
const getCorrespondingValidator = (serviceName) => {
  return serviceStore.allServices.find(
    (e) => e.service === serviceName + "ValidatorService"
  );
};

const getCorrespondingConsensus = (serviceName) => {
  return serviceStore.allServices.find(
    (e) => e.service === serviceName + "BeaconService"
  );
};
  ...
  switch (clickStore.selectedPreset.name) {
    ...
    case "staking":
      filter = (item) =>
        ...
        !/(SSVNetwork|Web3Signer|Charon|L2Geth|OpGeth|OpNode|OpReth|OpErigon)/.test(
          item.service
        );
    ...
    case "lidossv":
      filter = (item) => {
        if (element.category === "validator") {
          ...
          return (
            item.category === element.category &&
            !/(L2Geth|OpGeth|OpNode|OpReth|OpErigon)/.test(item.service)
          );
        }
        return false;
      };
    ...
    case "lidoobol":
      filter = (item) => {
        if (element.category === "execution" && element.service !== "CharonService") {
          ...
          element.category === "validator" &&
          element.service !== "CharonService"
        ) {
          return (
            item.category === element.category &&
    ...
    case "stereum on arm":
      filter = (item) =>
        item.category === element.category &&
        !/(Prysm|Reth|SSVNetwork|Web3Signer|Charon|L2Geth|OpGeth|OpNode|OpReth|OpErigon)/.test(
          item.service
        );
      if (manageStore.currentNetwork.network == "gnosis") {
        filter = (item) =>
    ...
    case "op full node":
      filter = (item) => {
        if (manageStore.currentNetwork.network == "op-mainnet") {
          return (
            item.category === element.category &&
            /(OpGethService|OpErigonService)/.test(item.service)
          );
        } else if (
          element.service === "OpGethService" ||
          element.service === "OpErigonService" ||
          element.service === "OpRethService"
        ) {
          return (
            item.category === element.category &&
            /(OpGethService|OpErigonService|OpRethService)/.test(item.service)
          );
        ...
        if (
          element.category === "consensus" &&
          element.service === "OpNodeBeaconService"
        ) {
          return item.category === element.category && /(OpNode)/.test(item.service);
        }
        return (
          ...
          /OpGethService|OpNodeBeaconService/.test(item.service)
        );
      };
      break;
      ...
      filter = (item) => {
        if (element.category === "execution") {
          ...
          if (
            ...
            /Op(Geth|Erigon|Reth)Service/.test(element.service)
          ) {
            return (
              item.category === "execution" &&
              /(OpGethService|OpErigonService)/.test(item.service)
