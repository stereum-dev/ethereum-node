<template>
  ...
  >
    ...
    >
      ...
      >
        <span
          class="col-start-1 col-span-2 text-sm text-gray-200 font-semibold uppercase"
          >{{ $t("importingSyncing.client") }}</span
        >
        <span
          class="col-start-4 col-span-2 text-sm text-gray-200 font-semibold uppercase"
          >{{ $t("importingSyncing.syncSrc") }}</span
        >
      </div>
      <div
...
export default {
  ...
  computed: {
    ...
    nextRouteHandler() {
      if (!this.btnActive) {
        return this.checkPointSync !== "" || !this.checkPointSync
          ? "importingVerify"
          : "disabled";
  ...
  methods: {
    ...
    filterServices() {
      this.executionClient = this.configServices.filter(
        (service) => service.category === "execution"
      )[0];

      this.consensusClient = this.configServices.filter(
